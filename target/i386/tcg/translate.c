...
/*
 * As DISAS_JUMP, but the jump is near: it changes EIP only, leaving cs_base
 * and the translation flags exactly as they were at the start of this TB.
 * That lets the TB lookup reuse the state the front end already has instead
 * of reconstructing it from the CPU.
 */
...
gen_eob(DisasContext *s, int mode)
{
    ...
    if (mode == DISAS_EOB_RECHECK_TF) {
        ...
        gen_helper_single_step(tcg_env);
               ...
               !inhibit_reset) {
        /*
         * s->flags is only still accurate for the destination if nothing
         * above has changed the state it records.  gen_reset_hflag() keeps
         * s->flags in step, but the RF reset above writes eflags directly,
         * so fall back to reading the state back out of the CPU there.
         */
#ifndef TARGET_X86_64
        if (mode == DISAS_JUMP_STATIC && !(s->flags & HF_RF_MASK)) {
