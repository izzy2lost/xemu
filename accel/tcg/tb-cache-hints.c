...
static char         *save_target_path;
static uint32_t      save_target_game_hash;
static int           last_autosave_count;
...
{
    g_free(save_target_path);
    save_target_path = NULL;
    save_target_game_hash = game_hash;
    last_autosave_count = 0;

    if (path && path[0] != '\0') {
        save_target_path = g_strdup(path);
    }
}
...
void tb_cache_record_hint(const TranslationBlock *tb)
{
    ...
    if (save_target_path && recorded_count >= 100 &&
        recorded_count - last_autosave_count >= 256) {
        tb_cache_save(save_target_path, save_target_game_hash);
        last_autosave_count = recorded_count;
    }
}
