                                ...
                                bool bypass_reentrancy_guard)
{
    do {
        ...
        if (bypass_reentrancy_guard) {
            r = memory_region_dispatch_read_lockless(mr, mr_offset, &val,
                                                     this_mop, full->attrs);
        } else {
                               ...
                               int mmu_idx, MMUAccessType type, uintptr_t ra)
{
    ...
    /* A whole-region lockless device already has the guard disabled. */
    if (mr->lockless_io) {
        return int_ld_mmio_beN(cpu, full, ret_be, addr, size, mmu_idx,
                               type, ra, mr, mr_offset, false);
    }

    if (mr->lockless_read && mr->lockless_read(mr->opaque, mr_offset, size)) {
        return int_ld_mmio_beN(cpu, full, ret_be, addr, size, mmu_idx,
                               ...
                               int mmu_idx, uintptr_t ra)
{
    ...
    if (mr->lockless_io ||
        (mr->lockless_read && mr->lockless_read(mr->opaque, mr_offset, size))) {
        bool bypass = !mr->lockless_io;

        a = int_ld_mmio_beN(cpu, full, ret_be, addr, size - 8, mmu_idx,
                            MMU_DATA_LOAD, ra, mr, mr_offset, bypass);
