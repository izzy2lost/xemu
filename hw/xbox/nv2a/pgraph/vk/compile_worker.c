...
static void process_pipeline_job(PGRAPHVkState *r, CompileJob *job)
{
    ...
    } else if (p->layout != VK_NULL_HANDLE) {
        vkDestroyPipelineLayout(p->device, p->layout, NULL);
    }

    for (int i = 0; i < p->num_module_infos; i++) {
        if (p->module_infos[i]) {
            pgraph_vk_unref_shader_module(r, p->module_infos[i]);
        }
    }
...
void pgraph_vk_compile_worker_init(PGRAPHVkState *r)
{
    ...
    r->compile_worker.initialized = true;
...
{
    if (!r->compile_worker.initialized) {
        return;
    }
    ...
    while ((job = QSIMPLEQ_FIRST(&r->compile_worker.queue)) != NULL) {
        QSIMPLEQ_REMOVE_HEAD(&r->compile_worker.queue, entry);
        if (job->type == COMPILE_JOB_PIPELINE) {
            PipelineCreateParams *p = &job->pipeline.params;
    ...
    r->compile_worker.initialized = false;
}
