                                             ...
                                             int height)
{
    ...
    /*
     * Upstream drives r->command_buffer directly and follows up with a raw
     * vkQueueWaitIdle. This fork has a render thread that owns the queue, so
     * go through the aux command buffer helpers instead -- they park the
     * render thread before submitting and wait on a fence rather than the
     * whole queue.
     */
    ...
    vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                      fan_pipeline);
    vkCmdDraw(cmd, 4, 1, 7, 0);

    vkCmdEndRenderPass(cmd);

    // Synchronize and transition framebuffer for copying to CPU
    pgraph_vk_transition_image_layout(pg, cmd, offscreen_image,
    ...
    vkCmdCopyImageToBuffer(cmd, offscreen_image,
                           VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, cpu_buffer, 1,
                           &region);

    /* Ends, submits and waits on the aux fence. */
