...
/*
 * Shader bodies carry no #version line -- build_shader_source() prepends the
 * right one, because this fork also runs the GL renderer on GLES where the
 * probe needs "#version 320 es" plus explicit precision qualifiers.
 */
...
static char *build_shader_source(const char *body)
{
#ifdef __ANDROID__
    /*
     * Geometry shaders are core in GLSL ES 3.20. determine_gpu_properties()
     * refuses to probe below that, so no #extension directive is needed here.
     */
    return g_strdup_printf("#version 320 es\n"
                           "precision highp float;\n"
                           "precision highp int;\n"
                           "\n%s",
                           body);
#else
    return g_strdup_printf("#version 400\n\n%s", body);
#endif
}

static GLuint compile_shader(GLenum type, const char *body)
{
    g_autofree char *source = build_shader_source(body);
...
#ifdef __ANDROID__
    /*
     * The probe itself needs a geometry shader. Where we can't compile one,
     * pgraph_glsl_gen_geom() is never used either (gl/shaders.c drops the
     * stage when geometry_shaders_supported is false), so leaving the winding
     * at the identity default is correct rather than merely safe.
     */
    if (!r->geometry_shaders_supported || r->gles_version < 320) {
        __android_log_print(ANDROID_LOG_INFO, "hakuX",
                            "geometry shader winding: probe skipped "
                            "(geom_shader=%s glsl_es=%d), using identity",
...
#ifdef __ANDROID__
    __android_log_print(ANDROID_LOG_INFO, "hakuX",
                        "geometry shader winding: tri=%d strip=%d/%d fan=%d",
                        pgraph_gl_gpu_properties.geom_shader_winding.tri,
                        pgraph_gl_gpu_properties.geom_shader_winding.tri_strip0,
                        pgraph_gl_gpu_properties.geom_shader_winding.tri_strip1,
                        pgraph_gl_gpu_properties.geom_shader_winding.tri_fan);
